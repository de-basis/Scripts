...
def count_occurrence(findings):
    counts=[]
    for i in findings:
        result=run("select count(concat({},{})) from odwmdev.{} where concat({},{})={}".format(i[5][0],i[5][1],i[3],i[5][0],i[5][1],i[4]))
...
def main():
    ...
    counts=count_occurrence(result)
    #print("*******Branches & Accounts Found******")
    with open('./report.txt','w+') as f:
        f.write("******Identified AG information******\n")
        f.write("AG_NAME|AGID|AGID_NAME|TABLES|FIELDS|TABLE_COUNT\n")
        for i in ag_info:
            f.write(f"{i}\n")
        f.write("******Branch+Account information******\n")
        for i in result:
            f.write("Br+Acc: {} |Table: {} |AGID: {} |AG_NAME: {} |Fields: {} |Date: {}\n".format(i[4],i[3],i[1],i[0],i[5],i[6]))
            #print("Br+Acc: {} |Table: {} |AGID: {} |AG_NAME: {} |Fields: {}".format(i[4],i[3],i[1],i[0],i[5]))
            #logger.info("Br+Acc: {} |Table: {} |AGID: {} |AG_NAME: {} |Fields: {}".format(i[4],i[3],i[1],i[0],i[5]))
        #print("*******Occurences*******")
        f.write("******Count of Branch+Account Occurences******\n")
        today=datetime.date.today()
        for i in counts:
            date_str=str(i[3])
            print(type(i[3]),"  ",date_str)
            try:
                ...
                f.write("Br+Acc:{}  Table:{}  Count: {} LoadDate: {} Years_since_Loaded: {:.2f}\n".format(i[0],i[1],i[2],i[3],years))
            #print("Br+Acc:{}  Table:{}  Count: {}".format(i[0],i[1],i[2]))
            #logger.info("Br+Acc:{}  Table:{}  Count: {} LoadDate: {}".format(i[0],i[1],i[2]),i[3])    
